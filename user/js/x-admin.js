layui.use(["element"], function() {
  ...
  $(".layui-nav-child").click(function() {
	$(this).find($(".layui-nav-child3")).siblings().removeClass("layui-nav-childed");
	$(this).find($(".layui-nav-child3")).addClass("layui-nav-childed");
  });
  ...
  element.on("nav(side)", function(elem) {
    ...
    res = element.tabAdd("x-tab", {
      title: title, //用于演示
      content:
        '<iframe frameborder="0" src="' + url + '" class="x-iframe"></iframe>'
    });
